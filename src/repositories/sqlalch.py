...
class QuestionDBRepository:
    ...
    async def filter_duplicated_questions(self, questions: list):
        ...
        stmt = select(Question).where(
            Question.id.in_(question.id for question in questions),
        )
        existing_question_ids = [
            question.id
            for question in (await self.session.execute(stmt)).scalars().fetchall()
        ]
        return list(
            filter(
                lambda question: question.id not in existing_question_ids, questions
            ),
        )
